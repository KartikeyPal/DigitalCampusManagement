...
const Input = ({ placeholder, value, type, onChange, visible, hidePassword, passwordToggleButton, required, containerClassName, className, labelClassName }) => {
  return (
    <div className={`relative min-w-[200px] w-[75%] group ${containerClassName || ''}`}>
      <input
        ...
        value={value}
        onChange={onChange}
        className={` m-2 peer w-full bg-transparent placeholder:text-slate-400 text-slate-700 text-sm border border-slate-200 rounded-md px-3 py-2 transition duration-300 ease hover:outline-none hover:border-slate-300 shadow-sm hover:shadow ${className || ''}`}
        required={required}
      />
